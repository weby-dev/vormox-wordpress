...
{
    /**
     * Backend statuses that mean the machine is ready.
     *
     * @var string[]
     */
    private const COMPLETED_STATUSES = ['COMPLETED', 'SUCCESS', 'ACTIVE'];
    ...
    public function create(Provider $provider, VmOrder $order): ProvisioningResult
    {
        ...
        $this->orders->update($order->id(), $identifiers);

        $status = strtoupper((string) $response->get('status', ''));

        $this->logger->info(
            sprintf('Creation request for order %d accepted with status %s.', $order->getWcOrderId(), $status),
            [
                'channel' => LogEntry::CHANNEL_PROVISIONING,
        ...
        if (in_array($status, self::COMPLETED_STATUSES, true)) {
            return ProvisioningResult::completed(
                __('The machine was created.', 'cloud-vm-manager'),
                $identifiers
            );
        }
